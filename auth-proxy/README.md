...
```
curl -X POST -H "X-DATASIGHT-USER:your_user" -H "X-DATASIGHT-ROLE:your_role" https://datasight-111111111.internal.clsconsole.tencentcls.com/api/user
```

> 域名说明：CLS DataSight 当前同时支持两套域名，**请勿直接照搬示例域名**，请先确认您实例的实际域名再替换：
> - **新域名**：`xxxxx(.internal)?.clsconsole.tencentcls.com` —— 后续新增实例只支持新域名
> - **老域名**：`xxxxx(.internal)?.clsconsole.tencent-cloud.com` —— 仅老实例可用，不会下线，但新增实例不会再分配
>
> 确认实例域名的方式：登录腾讯云 [CLS DataSight 控制台](https://console.cloud.tencent.com/cls) 在实例详情/域名信息中查看，或通过云API读取实例的域名字段。确认后，将本示例 curl 命令中的域名替换为您实例的实际域名即可，反向代理示例配置中也按同样方式替换 `proxy_pass` 域名。

1. 如果返回 

```
{"isLoggedIn":true,"domain":"datasight-111111111.internal.clsconsole.tencentcls.com","username":"your_user","role":"your_role","isAuthProxy":true}
```
